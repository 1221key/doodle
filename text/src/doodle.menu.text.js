(function ($) {
  ...
  // setup menu
  $.fn.kDoodle.menus.text = {
  	img: window.GlobalSpace.DiscussDomain + 'plugins/kDoodle/text/img/icons-menu-text.png',
    items: {
      bold: {
        icon: 'toggle',
        title: '粗体',
  ...
  $.fn.kDoodle.menus.main.items.text = {
    icon: 'menu',
    after: 'pencil',
    title: '文本',
    index: 7,
  ...
  $.fn.kDoodle.extend({
    generate: function () {
      ...
      this.$textInput = $('<textarea class="kDoodle-text-input" spellcheck="false"></textarea>')
     ...
     * setters
     ****************************************/
    setStrokeStyle: function (fillStyle) {
    ...
    _drawText: function () {
      var fontString = '',
          ...
          linesNew = [],
          textInputWidth = this.$textInput.width() - 2,//得到文本框宽度
          width = 0,
          lastj = 0,
      ...
      for (i = 0, ii = lines.length; i < ii; i++) {
        this.$textCalc.html('');//利用这个文本框作为一个临时的容器
        lastj = 0;
        //debugger
        for (j = 0, jj = lines[0].length; j < jj; j++) {//对单行文本大于文本框的进行拆解，拆成多行
          width = this.$textCalc.append(lines[i][j]).width();//每循环一次加一个字进去
